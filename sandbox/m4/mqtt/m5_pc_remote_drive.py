...
def main():
    ...
    #mqtt_client = None  # Delete this line, it was added temporarily so
    # that the code we gave you had no errors.
    ...
    root.bind('<Right>', lambda event: right(mqtt_client, left_speed_entry))
    ...
    back_button['command'] = lambda: back(mqtt_client, left_speed_entry,
                                                right_speed_entry)
    root.bind('<Down>', lambda event: back(mqtt_client, left_speed_entry,
                                            right_speed_entry))
